...
docker run --rm -v $OVPN_DATA:/etc/openvpn -i -e "EASYRSA_BATCH=1" -e "EASYRSA_REQ_CN=Travis-CI Test CA" $IMG ovpn_initpki nopass
...
#
docker exec -i -e "EASYRSA_BATCH=1" $NAME easyrsa build-client-full $CLIENT1 nopass
docker exec -i $NAME ovpn_getclient $CLIENT1 > $CLIENT_DIR/config.ovpn
docker exec -i $NAME bash -c "echo 'yes' | ovpn_revokeclient $CLIENT1"

# Determine IP address of container running daemon and update config
for i in $(seq 10); do
    SERV_IP_INTERNAL=$(docker inspect --format '{{ range .NetworkSettings.Networks }}{{ .IPAddress }}{{ end }}' "$NAME" 2>/dev/null || true)
...
#
docker exec -i -e "EASYRSA_BATCH=1" $NAME easyrsa build-client-full $CLIENT2 nopass
docker exec -i $NAME ovpn_getclient $CLIENT2 > $CLIENT_DIR/config.ovpn
docker exec -i $NAME bash -c "echo 'yes' | ovpn_revokeclient $CLIENT2"
