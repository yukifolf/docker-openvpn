...
# gen TCP client
docker run -v $OVPN_DATA:/etc/openvpn --rm -i $IMG easyrsa build-client-full $CLIENT_TCP nopass
...
docker run -v $OVPN_DATA:/etc/openvpn --rm -i $IMG easyrsa build-client-full $CLIENT_UDP nopass
